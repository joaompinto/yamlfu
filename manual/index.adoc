...
== Introduction
An yamlfu document is a valid YAML document extended with dynamic elements, in order to transform the document into a final YAML, yamlfu documents must be rendered using the  `yamlfu` utility.

CAUTION: yamlfu document rendering is equivalent to a python script execution. You should not bindly render documents obtained from untrusted sources.
